...
## Certificate Workflow Settings

Certificate workflow settings live in `certificateWorkflowSettings` and resolve through `convex/certificateWorkflowSettings.ts`. Broker orgs store broker defaults, client orgs may store client-admin-owned overrides, and the effective resolver applies client override → broker default → platform default. Certificate generation consumes the effective resolver for held certificate policy-change behavior instead of reading ad hoc organization certificate booleans directly; legacy organization flags remain as fallback defaults when no workflow settings row exists. Settings currently cover source-backed holder population, held certificate policy-change case creation, and renewal review-job creation/lead time.
