...
- Program administrator partners: MGAs, carriers, and underwriters that can approve or certify output are modeled as first-class partner organizations with `partnerKind: "program_admin"`. The registry in `partnerPrograms` matches policies through aliases against `mga`, `security`, `carrier`, and insurer fields, with policy-level overrides available for ambiguous extraction. Partner users have approval-scoped access through `/partner/approvals`, not broad broker/client workspace access.
...
- Program administrator certificate support lives in `partnerPrograms`, `coiTemplates`, `standingAuthorizations`, `certificateRequests`, and `certificateApprovals`. Template metadata supports PDF field mapping/standard overlay records; standing authorizations require a program, template, and optional policy/coverage constraints before Glass can auto-issue a certified certificate with a system approval record.
