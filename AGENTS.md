...
## Certificate Lifecycle Backfill

Generated certificate artifacts are being widened into a holder and lifecycle model. Legacy `certificates` rows remain the visible activity/audit source during rollout, with optional links to `certificateHolders`, `certificateParents`, and `certificateVersions` after backfill. `policyVersions` stores the current policy snapshot used by backfilled certificate versions. The maintenance entry points in `convex/certificateLifecycleBackfill.ts` support dry-run batch migration and paginated verification; duplicate legacy rows for the same org, policy, holder name, and holder address share one certificate parent with multiple versions, while rows missing a resolvable holder are marked ambiguous and left visible as legacy activity.
