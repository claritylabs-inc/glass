...
- `generateCoi.run` stores every newly generated COI PDF in Convex file storage and records it in `certificates` with `policyId`, `orgId`, certificate-holder text, source, creator, and storage file ID. `certificates.recordGenerated` also creates an issued `certificateVersions` row so later renewal and endorsement workflows can reason about latest issued versions. The policy detail page's Certificates tab reads this table; direct page generation uses `certificates.generateForPolicy`. Programmatic generation is exposed through REST (`GET/POST /api/v1/policies/:id/certificates`) and MCP/CLI tools (`list_policy_certificates`, `generate_policy_certificate`).
- Certificate renewal and post-endorsement reissue work is review-first. `certificateReviewJobs` tracks renewal/post-endorsement jobs, `certificateVersions` stores draft/issued/cancelled review versions, and `certificateReviewJobAudit` records job transitions. Renewal job creation scans active certificates with an issued latest version and creates draft review jobs rather than sending automatically. Endorsement completion resolves matching `certificateRequestHolds` and creates post-endorsement draft review jobs. Jobs stay `blocked_missing_holder_contact` until a holder email is recorded, and send/cancel/review mutations audit every transition.
