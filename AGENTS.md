...
- Default Fireworks language routes require `FIREWORKS_API_KEY` for direct provider calls. OpenAI vision-chat and embedding routes require `OPENAI_API_KEY`. Glass model routing is direct-provider-only; do not use Vercel AI Gateway as a fallback for chat, extraction, provisional extraction, classification, embeddings, web retrieval, or any configured model route.
