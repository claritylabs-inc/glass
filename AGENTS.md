...
8. Store `sourceNodes` with embeddings for retrieval, store `sourceSpans` for exact PDF highlights, and keep `sourceChunks` / `documentChunks` only as compatibility artifacts for old paths.
...
- The extract phase stores `documentChunksForEmbedding`, `sourceSpansForStorage`, `sourceNodesForStorage`, and `sourceChunksForEmbedding` in a storage-backed `embedding_payload` artifact before advancing to `embed_and_store`, so a resumed embedding phase can reload transient artifacts without inflating checkpoint documents. Artifact blobs are cleaned up after durable embedding/source-span/source-node storage succeeds, cancellation, terminal success, and full restart; generic errors keep artifacts for resume/retry.
...
- `sourceNodes` — canonical source-tree hierarchy and retrieval index (vector over node descriptions)
- `sourceSpans` — exact PDF evidence/highlight layer
- `sourceChunks` — compatibility source-span windows for policies not yet rebuilt into source nodes
...
- `buildDocumentContext()` — if source nodes exist, embed query and search `sourceNodes`, expand hierarchy context, and attach stable `sourceSpanIds`. If only legacy chunks exist, it queues `ensurePolicyV3SourceTree()` and warns that exact policy-wording answers require source-tree rebuild.
...
- SDK query-agent wrappers use [convexSourceRetriever.ts](convex/lib/convexSourceRetriever.ts) to search `sourceNodes`, return hierarchy-expanded packets with exact spans, and fall back to source spans only when no source-node evidence exists.
