...
- `npm run conductor:setup` — install Node 24 and dependencies, provision/import/seed a worktree-native Convex deployment, prepare local worker env, start Apple `container`, and build worktree-tagged worker images
...
- `npx convex run seed:seed` — idempotently seed the local-only Conductor fixture (operator, broker, client, relationship, and sample policy)
...
- Production and staging use `EXTRACTION_WORKER_MODE=external`, `EXTRACTION_WORKER_SECRET`, `EXTRACTION_WORKER_URL`, `EXTRACTION_WORKER_EXPECTED_PROTOCOL_VERSION=source-tree-v1`, and `EXTRACTION_WORKER_EXPECTED_CL_SDK_VERSION` matching the deployed `extraction-worker` `@claritylabs/cl-sdk` package spec. When bumping `@claritylabs/cl-sdk`, update the Convex expected-version env for each environment as that environment's extraction worker rolls; a mismatch makes Convex reject worker claims before leasing jobs and can leave uploads sitting at "Extraction starts shortly." Every Convex environment that serves agent tools also needs `MAPBOX_ACCESS_TOKEN` so `lookup_address` can permanently geocode postal addresses that may be saved on certificates.
- Each Conductor worktree owns a native local Convex deployment under `.convex/local/default/`; it does not use `acoustic-caiman-755` for ordinary local testing. Setup imports cloud-dev environment variables but not the cloud database, then seeds a fresh local database once with a curated shared-dev-derived fixture: `terry@claritylabs.inc` operator access, Montgomery Risk broker/admin access through `terry@montgomeryrisk.com`, Cove client/admin access through `adyan@cove.dev`, unique customer phone identities, their broker/client relationship, and one final sample policy. Montgomery Risk starts with `whiteLabelingEnabled: false`; the seed fetches the Montgomery Risk and Cove website favicons through the shared logo-import path and stores both in worktree-local Convex file storage. The configured `IMESSAGE_TERMINAL_FROM_PHONE` belongs to the broker admin; Spectrum `/as broker`, `/as client`, and `/as public` switch between tenant-scoped and anonymous test paths without restarting the worker. The seed is idempotent and restricted to `GLASS_ENV=local`; it does not copy auth sessions, mail, documents, files, or operational history. Setup also maps the copied root `NEXT_PUBLIC_MAPBOX_TOKEN` to local Convex `MAPBOX_ACCESS_TOKEN`, overrides `ALLOW_DEV_CLEAR=true`, email capture, terminal iMessage, local worker URLs, and unique worker secrets. The Conductor archive hook deletes `.convex/local/default/`; closing a tab without archiving preserves it. The shared dev deployment remains an integration lane. Production must not set dev-clear or terminal-iMessage flags.
...
- Generated worker runtime env files live under gitignored `.context/`. Setup forces terminal-mode iMessage locally, creates matching worktree-local secrets, and keeps real Photon credentials disabled. Worker images use `:conductor-<worktree-directory>` tags so builds cannot overwrite another worktree's default image; the directory slug is stable when the workspace or branch is renamed, so setup and runtime keep resolving the same local image.
...
- Extraction, query agent, and prompts: `@claritylabs/cl-sdk` 4.4.0 source-tree contract. Legacy v2-style `sourceSpans` remain read-compatible through the same canonical `sourceNodes` / `operationalProfile` storage shape.
...
- Broker workspaces own client setup, certificates, policy delivery, and broker follow-ups. Glass no longer has a separate General Agent tenant model, partner approval queues, partner programs, standing authorizations, or certified COI template overlays. Treat former MGA/program-administrator tenant workflows as broker-owned workflows. After deploying this removal, an operator can run `operator.cleanupRemovedProgramAdminData` once to delete legacy program-admin org rows and dropped program-admin tables.
- Broker contact identity for client orgs comes from `brokerClientAssignments`. Do not add static broker contact fields back to `organizations`; assignment rows carry the canonical producer/contact name, email, and phone used for broker follow-ups. Broker admins select and edit the client-facing producer on the client settings Broker contact tab; broker members and client users see the resolved contact read-only.
...
- Auth, authorization, and operator access: `convex/lib/access.ts` owns customer org access, broker-of-client access, connected-client read access, and `assertCan*` guards. `convex/lib/operatorIdentity.ts` owns operator profiles, bootstrap, impersonation, and operator audit. API and MCP auth use OAuth through `convex/lib/apiAuth.ts`, `convex/oauth.ts`, and the Convex HTTP handlers; thread authorization lives in `convex/lib/threadAccess.ts`. Do not add ad hoc auth checks inside feature modules when an access primitive exists.
...
- Public APIs and integrations: REST route behavior must follow `convex/lib/apiAuth.ts`, `convex/lib/apiDto.ts`, and `convex/lib/apiError.ts`. MCP behavior belongs in the Convex MCP HTTP handlers, with tools limited to current policy, thread, org, agent, broker/client, certificate, and connected-vendor primitives. Remote and local MCP clients both authenticate through OAuth; local clients use `mcp-remote` rather than a separate stdio server.
...
- `chat_vision` → OpenAI `gpt-5.6-terra` for image-bearing web chat turns that still need the full chat tool set; ordinary text-only turns remain on `chat`
...
- `extraction_coverage_recovery` → OpenAI `gpt-5.4-mini` by default for opt-in document-wide source-grounded recovery, with independent budgets and telemetry. Model failure returns control to Glass's deterministic recovery instead of escalating to another language model.
...
- Voice memos use OpenAI's bounded-file transcription endpoint through `transcribeAudioForOrg` / `transcribeAudioForPublicTask`, not the language-model generation helpers. `voice_transcription` selectors expose only audio-transcription models; do not route this task to `chat_vision` because the Responses path used by that route accepts text/image input rather than audio files.
...
- Web chat streaming in [processThreadChat.ts](convex/actions/processThreadChat.ts) stays on the Fireworks DeepSeek V4 Flash route for text-only turns and switches to the image-capable `chat_vision` route when the built message history contains an image part. Image-bearing retries may use a configured fallback only when that fallback is also image-capable; otherwise they retry the selected vision route. Text-only turns keep the existing transient `server_error` / 5xx retry behavior before any visible text, tool call, or side-effectful work has started.
...
- It requires confirmation instead of sending when the recipient email is inferred or unknown, the body/subject is incomplete, attachments are ambiguous, or `autoSendEmails` is false and the user has not explicitly approved the exact draft. Clicking Send or issuing an explicit typed, iMessage, MCP, or CLI send command confirms the exact persisted draft; it does not bypass missing recipient/subject/body fields or COI attachment safety guards. Web-chat approval phrases send deterministically only when the immediately preceding response is linked to that one persisted draft, so an unsupported claim that a different or revised draft is ready cannot send a stale artifact. Agent-approved sends update and transition the existing thread-and-recipient draft to pending/sent rather than creating a second active row; scheduling also cancels any older duplicate drafts for that recipient. Agent prose must not claim a draft was updated or an email was sent unless the email tool result confirms it.
...
Application, quote, passport, business-context, and integration tools are gone. The MCP endpoint exposes only current policy, thread, org, agent, broker/client, certificate, and connected-vendor tools.

All MCP tool invocations require an OAuth bearer token and resolve org from session metadata. Write tools require a token with `write` scope.
