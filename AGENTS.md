...
- Stored declaration-fact replacement is owned by `convex/declarationFacts.ts`; `convex/backfillDeclarationFacts.ts` rebuilds those facts from final stored policies without scheduling model extraction. Both paths must remain idempotent and must resync only the insured/client profile fact allowlist.
