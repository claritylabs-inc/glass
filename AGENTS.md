...
- Production and staging use `EXTRACTION_WORKER_MODE=external`, `EXTRACTION_WORKER_SECRET`, `EXTRACTION_WORKER_URL`, `EXTRACTION_WORKER_EXPECTED_PROTOCOL_VERSION=source-tree-v1`, and `EXTRACTION_WORKER_EXPECTED_CL_SDK_VERSION` matching the current `extraction-worker` `@claritylabs/cl-sdk` package spec.
...
Glass is an insurance intelligence platform built on Next.js + Convex. v0.2.0 is a deliberate simplification: the product is now focused on policies, an agentic chat assistant, an agentic inbound email agent, lightweight per-org knowledge store (`orgMemory`), and broker-managed application intake. The old Applications v2, Client Passport / ACORD 125, email inbox scanning, org context documents, and Merge.dev sync backend have all been removed.
...
- Application intake: brokers can manage reusable templates and per-client intake runs from `/applications` and client detail application tabs. Current tables are `applicationTemplates`, `applicationIntakes`, `applicationMessages`, `applicationContextProposals`, and `applicationPackets`; do not resurrect the old Passport/Application v2 JSON-blob model. Reusable question graph, planning, context proposal, and packet assembly logic belongs in `@claritylabs/cl-sdk`; Glass owns Convex persistence, routing, permissions, broker/client UI, notifications, and activity rows. Existing clients can start or continue intake through authenticated web chat, known inbound email, linked iMessage/SMS, MCP chat, or broker/client portal actions. New-client intake V1 is broker-specific email-handle only; unknown shared-number iMessage/SMS prospects stay in the public demo/sales flow until broker-specific numbers exist. Packets are marked broker-ready for external carrier submission; Glass does not auto-submit to carriers in V1.
...
- Agent tools: web chat, inbound email, iMessage, and MCP chat expose `lookup_connected_vendors`, `lookup_vendor_policies`, and `lookup_vendor_compliance` so agents can answer vendor-specific compliance questions with the actual vendor roster, vendor policies, and requirement-by-requirement diffs instead of relying only on the generic requirements summary. They also expose `start_application_intake`, `answer_application_questions`, `check_application_status`, and `prepare_application_packet` for existing-client application workflows. MCP write-capable API-key callers may use these shared tools through `ask_glass`; read-only MCP identities receive a tool-level refusal for write operations.
...
- Clients can ask for policy updates in plain language, including standalone client orgs without a connected broker. Broker email drafting remains broker-mediated: use connected broker identity first, then manual broker identity, then explicit user-provided broker contact, and keep sending blocked until a broker recipient is known.
- If a policy matches an active program administrator partner, approval is routed to that partner's approval queue. Partner approval marks the hidden follow-up complete and stores a staged policy update for broker/admin review or updated-document import; Glass does not automatically patch structured policy fields from approval in v1.
