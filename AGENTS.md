...
- `generateCoi.run` stores every newly generated COI PDF in Convex file storage and records it in `certificates` with `policyId`, `orgId`, certificate-holder text, source, creator, and storage file ID. Certificate generation routes through `certificates.generateForOrg`, which resolves a per-org `certificateHolders` row, a current `policyVersions` row, a stable `policyCertificates` parent for `policyId + certificateHolderId`, and issued `certificateVersions` rows. By default, requesting the same holder for the same current policy version returns the latest existing version metadata instead of producing a duplicate PDF; callers must pass `forceReissue` to create another issued version under the same parent. Legacy `certificates` rows remain the attachment/file compatibility record for UI, REST, MCP, CLI, chat, email, and iMessage callers. The policy detail page's Certificates tab reads this table; direct page generation uses `certificates.generateForPolicy`. Programmatic generation is exposed through REST (`GET/POST /api/v1/policies/:id/certificates`) and MCP/CLI tools (`list_policy_certificates`, `generate_policy_certificate`).
