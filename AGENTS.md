...
Glass is an insurance intelligence platform built on Next.js + Convex. v0.2.0 is a deliberate simplification: the product is now focused on post-binding policies, renewals, an agentic chat assistant, an agentic inbound email agent, lightweight per-org knowledge store (`orgMemory`), and compliance workflows. Sales, application intake, unbound quote storage, the old Applications v2, Client Passport / ACORD 125, email inbox scanning, org context documents, and Merge.dev sync backend have all been removed from Glass.
...
- App shell and navigation: `components/app-shell.tsx`, `components/app-sidebar/*`, and `components/app-sidebar/nav-config.tsx` own sidebar layout, menu item styling, shortcuts, connect-feature menu visibility, and settings sidebar insertion. Settings section identity belongs in `lib/settings-sections.ts`; do not create page-local settings nav arrays.
- Operational UI: use `components/ui/operational-panel.tsx` for dense panels, label/value rows, repeated operational sections, and operator-friendly information display. Use `components/ui/pill-button.tsx` for pill-shaped actions, including anchors and download links. Use `components/ui/select.tsx`, `components/ui/dropdown-menu.tsx`, or `components/ui/searchable-select.tsx` instead of native selects. Use `components/settings/settings-switch.tsx` for settings toggles and `components/settings/feature-flag-toggle-row.tsx` for feature-flag rows.
- Brand and identity UI: `components/ui/brand-icon.tsx`, `components/ui/org-brand-icon.tsx`, `components/ui/logo-icon.tsx`, `lib/branding.ts`, `lib/viewer-branding.ts`, `convex/lib/branding.ts`, and `convex/lib/orgBranding.ts` own app, org, broker, and email branding presentation. Broker white-labeling takes priority over client branding; client logos are still visible when no broker override applies.
...
- Current org context and cached app data: `lib/sync/use-cached-query.ts`, `lib/sync/glass-cached-queries.ts`, `lib/sync/operator-cached-queries.ts`, and `lib/sync/use-local-first-auto-save.ts` own local-first query caching and optimistic updates. Active org resolution should use the existing `useCurrentOrg` owner for the surface being touched rather than creating a page-local viewer/org query.
...
- Policy extraction and source evidence: `extraction-worker/` owns external LiteParse/PDF.js preprocessing and policy extraction execution. Convex orchestration lives in `convex/actions/policyExtraction.ts`, `convex/lib/extraction.ts`, `convex/lib/pipelineMutations.ts`, `convex/lib/sourceTree.ts`, `convex/lib/policyDocumentStructure.ts`, `convex/lib/extractionPostProcess.ts`, `convex/lib/declarationFacts.ts`, `convex/lib/coverageBreakdown.ts`, `convex/lib/coverageNames.ts`, `convex/lib/coverageScoping.ts`, and `convex/lib/sdkCallbacks.ts`. Use source spans/source nodes/operational profile as the evidence path; do not reintroduce legacy PDF map or raw-PDF model paths.
- Policies, endorsements, and broker follow-ups: policy rows, versions, and source-backed changes live behind `convex/policies.ts`, `convex/lib/policyVersioning.ts`, `convex/lib/policyLookup.ts`, `convex/lib/policyTypes.ts`, and policy-change helpers under `convex/lib/policyChange*.ts`. Broker follow-up workflow primitives live in `convex/lib/workflows/brokerFollowUp.ts`; product copy should say broker follow-up, policy update, or endorsement rather than rebuilding policy-case UI.
- Certificates: `convex/certificates.ts`, `convex/lib/workflows/certificateRequest.ts`, `convex/lib/certificateRequestGate.ts`, `convex/lib/certificateHolderPopulation.ts`, `convex/lib/certificateIdentity.ts`, and `convex/lib/coiGenerator.ts` own COI lifecycle, holder records, additional-insured gating, versioning, and generated PDFs. Do not add MGA/program-admin, certified COI, partner approval, or template-overlay primitives back into Glass.
- Compliance and Connect: compliance requirements belong in `convex/actions/complianceRequirements.ts`, `convex/lib/requirementSemantics.ts`, `convex/lib/complianceAgent.ts`, and `convex/lib/vendorComplianceTools.ts`. Connect vendor/client relationships belong in `convex/connectedOrgs.ts` and the `/connect/*` surfaces. The `connect_features` feature flag gates Connect pages and vendor requirements UI for client orgs.
- Agent, chat, and task control: prompts and shared agent rules live in `convex/lib/agentPrompts.ts`; scope/context in `convex/lib/agentScope.ts`; tool definitions/execution/audit in `convex/lib/chatTools.ts`, `convex/lib/agentToolExecutors.ts`, and `convex/lib/agentToolAudit.ts`; thread history in `convex/lib/agentMessageHistory.ts`; deterministic reset/cancel controls in `convex/lib/taskControlIntent.ts`, `convex/lib/taskControlDecision.ts`, `convex/lib/webChatDeterministicControls.ts`, and `convex/lib/textChannelControls.ts`.
- Email, notifications, and iMessage: outbound mail must go through `convex/lib/resend.ts`. Email HTML shells live in `convex/lib/emailTemplate.ts`, notification emails in `convex/lib/notificationEmailTemplate.ts`, inbound email agent paths in `convex/lib/emailSubagent.ts` and `convex/lib/emailIntentGuards.ts`, notification behavior in `convex/lib/notificationTypes.ts` and `convex/lib/notify.ts`, and iMessage/Spectrum paths in `convex/lib/imessage*.ts` plus `imessage-worker/`. Do not expose notification channels in UI unless the corresponding sender path exists.
...
- Default Fireworks language routes require `FIREWORKS_API_KEY` for direct provider calls. OpenAI embedding routes still require `OPENAI_API_KEY`. Main Glass language routing and the extraction worker are direct-provider-only; do not use Vercel AI Gateway as a fallback for chat, extraction, provisional extraction, classification, or configured model routes. Vercel AI Gateway is isolated to web-retrieval routes that explicitly support it.
...
Glass now has a top-level compliance workflow for contractor/vendor insurance monitoring. Requirement imports can also create source-backed requirement records from leases, client contracts, vendor requirement packets, or other documents. Uploaded or pasted sources are recorded in `requirementSourceDocuments`, and created `insuranceRequirements` keep `sourceDocumentId`, `sourceDocumentName`, `sourceType`, `sourceExcerpt`, and optional source page fields so web UI, agents, MCP, CLI, and REST responses can explain the original source language behind a requirement. The same import surface powers bulk requirement import and lease/contract extraction. `appliesTo` means obligation ownership and tab placement: extracted requirements for the org's own obligations belong in **My requirements** (`appliesTo: own_org`), while vendor/customer standards remain in **Vendor requirements**. Do not use `appliesTo` to decide what evidence proves the row.

`insuranceRequirements.evaluationTarget` means what evidence can satisfy the requirement: `own_policy`, `connected_vendor_policy`, `subcontractor_policy`, `manual_control`, or `not_policy_checkable`. Customer-imposed obligations can be `appliesTo: own_org` while still requiring subcontractor/downstream evidence; those rows must not be marked met from the current organization's policy. Requirement records deliberately mirror the policy `coverages` shape (`name`, `coverageCode`, `limit`, numeric `limitAmount`, limit typing, deductible fields, and `originalContent`) so policy-checkable rows can compare against extracted policy coverage data. Requirements can be created one at a time or bulk-generated from pasted text / uploaded requirement documents through `convex/actions/complianceRequirements.ts`, which extracts text from TXT/Markdown/PDF/DOCX/CSV/JSON inputs and uses Glass's static chat route to produce coverage-shaped structured requirements plus row-level evaluation semantics. PDF requirement imports use local PDF.js text extraction. Current web and MCP surfaces compute live checklist status from active connected vendors plus extracted `policies` data. The daily `vendorComplianceMonitor` cron records deterministic snapshots in `vendorComplianceChecks`, creates client notifications and notification emails for new or recurring compliance gaps, drafts vendor follow-up emails when a vendor contact is available, and sends iMessage/SMS alerts to org admins with phone numbers when the worker is configured. The deterministic checker matches only policy-checkable rows against policy types, summaries, coverages, expiration dates, structured coverage limits, and insured names, returning `met`, `missing`, `expiring_soon`, `expired`, or `needs_review`; manual/subcontractor/control rows return `needs_review` until the right evidence is provided. Future LLM review should augment this table rather than replacing requirement ownership and evidence-target semantics.
...
## Policy Change Broker Follow-Ups And Endorsements

Policy changes are treated as simple broker follow-ups, not a policy-page case-management surface. The user-facing flow should feel like: "draft/email my broker to update the policy with XYZ." `policyChangeCases` remains as hidden plumbing for thread continuity, broker-recipient resolution, outbound-email correlation, inbound broker replies, and endorsement attachment. Do not rebuild a policy-scoped Requests tab or expose packet/status internals unless a user explicitly asks for audit data.

Shared agent tools across web chat, inbound email, iMessage, MCP, and CLI ask paths are `create_policy_change_request`, `add_policy_change_info`, `draft_policy_change_email`, and `complete_policy_change_from_endorsement`. Keep the tool names for compatibility, but product copy should say broker follow-up, policy update, or endorsement. Broker recipient emails must come from explicit user input or known contacts; otherwise Glass asks for the recipient before sending.
...
`policyDeclarationFacts` stores normalized comparable declaration and policy facts with provenance fields for policy-change and endorsement update paths. Extraction completion syncs those facts but does not scan for cross-policy declaration discrepancies, generate declaration mismatch notifications, or run an LLM copy pass for policy-detail conflicts. Multiple active policy numbers, carriers, and producers are normal for package accounts and should not become proactive account-level warnings.
...
Outbound broker emails for policy updates are persisted as `pendingEmails` and email `threadMessages` with `policyChangeCaseId`. Sending assigns a deterministic RFC `Message-ID` and uses the thread-specific email address as Reply-To when available, so broker replies can be correlated back to the original thread and hidden follow-up record. Inbound broker replies matched by `In-Reply-To`, `References`, or policy-change email thread context are recorded on that follow-up, and the email agent is prompted to use `complete_policy_change_from_endorsement` for attached endorsement PDFs instead of importing them as standalone policy documents.
...
- Operational policy profile: `operationalProfile` stores source-backed metadata, parties, policy types, coverage lines, limits, deductibles, premiums, key dates, and endorsement support with `sourceNodeIds` and `sourceSpanIds`. Keep policy classification in `policyTypes`; do not add a parallel `coverageTypes` alias, `coverageOrigin` bucket, or deterministic coverage-origin projection.
...
Glass persists hidden policy-change case state for endorsement follow-ups:

- `policyChangeCases` stores request text, status, affected policy, broker-recipient snapshot, and IDs needed to correlate email replies and endorsement attachments.
- `caseMessages`, `caseEvidenceLinks`, and `caseValidationReports` remain available for audit/debug context, but they are not a primary user interface.
- `convex/policyChanges.ts` exposes safe entrypoints to capture a broker follow-up from chat, email, or uploaded documents, process replies, draft the broker email, cancel the follow-up, and complete it from received endorsement evidence.
- Web chat stores the created `policyChangeCases` ID on the producing `threadMessages` record. The thread renders a compact broker-follow-up card and right panel with only the requested change, status, policy/source metadata, missing recipient/details, held-COI context, and cancellation.
- Clients can ask for policy updates in plain language. Broker email drafting remains broker-mediated: use the broker assignment contact first, then explicit user-provided broker contact, and keep sending blocked until a broker recipient is known.
