...
- Default Fireworks language routes require `FIREWORKS_API_KEY` for direct provider calls. OpenAI embedding routes still require `OPENAI_API_KEY`; routes without a direct server key use Vercel AI Gateway through `AI_GATEWAY_API_KEY` or `VERCEL_OIDC_TOKEN`.
