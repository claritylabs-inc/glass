...
export function PromptReferenceTag({
  ...
  return (
    <span
      className={cn(
        "inline-flex h-[18px] max-w-[min(14rem,100%)] shrink-0 items-center gap-0.5 align-middle rounded-4xl bg-foreground/5 px-1.5 text-[0.625rem] font-medium leading-3 text-foreground/70",
        className,
      )}
    >
      ...
      {onRemove ? (
        <button
          ...
          onClick={onRemove}
          title={`Remove ${label}`}
          className="-mr-0.5 inline-flex h-3 w-3 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-foreground/8 hover:text-foreground"
        >
