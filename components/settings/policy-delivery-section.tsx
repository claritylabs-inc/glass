    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="space-y-1.5">
            <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>Order</span>
            <Input type="number" value={priority} onChange={(event) => setPriority(Number(event.target.value) || 100)} placeholder="100" />
          </label>
          <div className="space-y-1.5">
            <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>Enabled</span>
        ...
        <div className="space-y-1.5">
          <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>When matched</span>
          <ActionSelect value={action} onChange={setAction} />
        </div>
        <div className="space-y-1.5">
          <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>Send by</span>
        ...
        <label className="space-y-1.5">
          <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>Additional condition</span>
        ...
        <label className="space-y-1.5">
          <span className={`text-muted-foreground ${typeStyle("caption.default")}`}>Message instructions</span>
  ...
  return (
    <div className="space-y-4">
        ...
        {isInheritedClientSettings ? (
          <div className="space-y-5 px-5 py-5">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div>
                <p className={`text-muted-foreground ${typeStyle("caption.default")}`}>Automation</p>
              ...
              <div>
                ...
                <p className={`mt-1 text-foreground ${typeStyle("body.default")}`}>
                  {ACTION_LABELS[draft.defaultAction]}
                </p>
              </div>
              <div>
                <p className={`text-muted-foreground ${typeStyle("caption.default")}`}>Send by</p>
                <p className={`mt-1 text-foreground ${typeStyle("body.default")}`}>
          ...
          >
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <div className="space-y-2">
                <p className={`text-muted-foreground ${typeStyle("caption.default")}`}>Default outcome</p>
      ...
      <OperationalPanel>
        ...
        />
        {rules.length === 0 ? (
          <div className={`px-5 py-8 text-muted-foreground ${typeStyle("body.default")}`}>
            {clientOrgId
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                <TableHead className={`px-4 text-muted-foreground ${typeStyle("label.table")}`}>Rule</TableHead>
                <TableHead className={`text-muted-foreground ${typeStyle("label.table")}`}>Action</TableHead>
                <TableHead className={`text-muted-foreground ${typeStyle("label.table")}`}>Channels</TableHead>
                <TableHead className={`text-muted-foreground ${typeStyle("label.table")}`}>Order</TableHead>
                <TableHead className={`px-4 text-right text-muted-foreground ${typeStyle("label.table")}`}>Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {rules.map((rule) => (
                ...
                >
                  <TableCell className="px-4">
                    <div className="flex items-center gap-2">
                      <span className={`text-foreground ${typeStyle("body.medium")}`}>{rule.name}</span>
