...
/**
 * Confirm no user-facing "Prism" strings appear in MCP tool description fields.
 * Internal identifiers (PrismClient type references) are excluded.
 */
describe("MCP tool descriptions", () => {
  const toolFiles = readdirSync(TOOLS_DIR).filter((f) => f.endsWith(".ts"));

  for (const file of toolFiles) {
    it(`${file} has no user-facing "Prism" in description strings`, () => {
      ...
      for (const match of descriptionMatches) {
        expect(match[1]).not.toMatch(/Prism/i);
      }
    });
  }
});
