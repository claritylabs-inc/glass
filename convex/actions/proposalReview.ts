...
${compact({
      ...
      if (abortSignal.aborted) throw new Error("Proposal review took too long. Try again.");
      throw error;
    }
    const review = normalizeProposalReview(generated.object, {
      requirementIds: input.requirements.map((requirement: { _id: string }) => String(requirement._id)),
      specificationIds: input.specifications.map((specification: { _id: string }) => String(specification._id)),
      extractedOffer: input.extractedOffer,
    });
    ...
    return { ...saved, conclusion: review.conclusion, findingCount: review.findings.length };
  },
});
