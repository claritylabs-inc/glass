...
export const sendDraftInvite = action({
  ...
  handler: async (ctx, args) => {
    ...
    const subject = `${brokerName} invited you to Spot`;
...
<tr><td style="padding:28px 40px 0 40px;">
  <p class="spot-email-text-secondary" style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;font-size:15px;color:#374151;line-height:1.6;">
    <strong>${brokerName}</strong> has invited you${draft.name ? ` (<strong>${draft.name}</strong>)` : ""} to Spot — a shared workspace for your policies and documents.
  </p>
</td></tr>
${messageBlock}
<tr><td align="center" style="padding:24px 40px 0 40px;">
  <a href="${inviteUrl}" class="spot-email-button" style="display:inline-block;padding:8px 22px;background-color:#000000;color:#ffffff;font-family:-apple-system,sans-serif;font-size:14px;font-weight:500;text-decoration:none;border-radius:999px;line-height:1.4;">Accept invitation</a>
</td></tr>
<tr><td style="padding:20px 40px 0 40px;">
  <p class="spot-email-text-muted" style="margin:0;font-family:-apple-system,sans-serif;font-size:12px;color:#6b7280;line-height:1.6;">
    Or copy this link:<br><a href="${inviteUrl}" class="spot-email-link" style="color:#6b7280;word-break:break-all;">${inviteUrl}</a>
  </p>
</td></tr>
<tr><td style="padding:16px 40px 32px 40px;">
  <p class="spot-email-text-muted" style="margin:0;font-family:-apple-system,sans-serif;font-size:11px;color:#9ca3af;">This invitation expires in 14 days.</p>
</td></tr>`;
  ...
  handler: async (ctx, args) => {
    ...
    if (!reused) {
      ...
      await notify(ctx, {
        ...
        body: `${orgDoc?.name ?? "A client"} accepted your invitation and joined Spot.`,
        relatedOrgId: clientOrgId,
        actionType: "view_client",
        actionPayload: { clientOrgId },
      });
