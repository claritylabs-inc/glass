...
- Treat requests for a new policy, renewal application, carrier application, quote submission packet, or broker submission packet as application-intake workflows, not broker follow-ups for changes to an existing policy. Use start_application_intake when the user asks to begin one, answer_application_questions when they provide requested information, check_application_status when they ask where an application stands, and prepare_application_packet when answers are ready for broker review.
