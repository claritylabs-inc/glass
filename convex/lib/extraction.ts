...
import { METADATA_PROMPT, buildSectionsPrompt, buildSupplementaryEnrichmentPrompt } from "./prompts";
...
async function extractChunkWithRetry(
  ...
  const chunkRaw = await callClaude(
    anthropic, pdfBase64, buildSectionsPrompt(start, end), 8192, HAIKU_MODEL, log,
  );
  try {
      ...
      if (pageSpan > smallerSize) {
        ...
        for (const [subStart, subEnd] of subChunks) {
          const subResults = await extractChunkWithRetry(
            anthropic, pdfBase64, subStart, subEnd, nextSizeIndex, log,
          );
    ...
    const sonnetRaw = await callClaude(
      anthropic, pdfBase64, buildSectionsPrompt(start, end), 16384, SONNET_MODEL, log,
    );
    try {
...
async function extractSectionChunks(
  ...
  for (const [start, end] of chunks) {
    const results = await extractChunkWithRetry(anthropic, pdfBase64, start, end, 0, log);
...
export async function extractFromPdf(
  ...
  const sectionChunks = await extractSectionChunks(anthropic, pdfBase64, pageCount, log);
