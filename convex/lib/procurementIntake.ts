...
export type ProcurementIntakeExtraction = z.infer<typeof procurementIntakeExtractionSchema>;
...
export function normalizeProcurementIntake(value: ProcurementIntakeExtraction) {
  const requirements = value.insuranceObligations.map((item) => ({
    proposedRequirement: {
      ...
      provisions: [...new Set((item.provisions ?? []).map((entry) => entry.trim()).filter(Boolean))],
      requiredForms: [...new Set((item.requiredForms ?? []).map((entry) => entry.trim()).filter(Boolean))],
  ...
  const specifications = value.placementSpecifications.map((item) => ({
    key: item.key.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, ""),
