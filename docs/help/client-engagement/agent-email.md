...
Agent email lets approved people work with Glass from an ordinary email thread.
...
address**. It normally looks like `<workspace>@glass.insure`. Always use the
address displayed in Glass rather than guessing a handle or using an older
domain.

An administrator can turn **Available by email** on or off. For a managed
client, the broker administrator controls who Glass recognizes as belonging to
that client.
...
## Ways to include Glass

### Email Glass directly

Put the Glass address in **To**, describe the request, and attach any relevant
documents. Glass replies to the sender.

### Copy Glass on a conversation

Put the Glass address in **Cc** when you want it to help within an existing
conversation. Glass keeps the relevant participants on the reply, so review the
recipient list before copying it onto a sensitive thread.

Put Glass in **To** or **Cc**, not **Bcc**. Inbound routing uses the visible
recipient list to select the correct agent and workspace.

### Forward a request

An approved team member can forward a request to Glass. When the original
sender can be identified safely, Glass can prepare the response for the
original conversation while keeping the forwarding team member involved. Add a
short instruction above the forwarded content so Glass knows what outcome you
want.

### Continue the same request

Reply in the existing email chain. Glass uses email reply headers and the
conversation subject to preserve the thread. Starting a new message with a new
subject may create a separate Glass thread.
...
Glass may use an attachment to answer the email and, when requested or clearly
...
## What Glass can do by email

Glass can look up policies and source evidence, compare coverage, check
...
Email is an AI-only Glass surface. A human operator does not silently join the
...
recipients before sharing private information. Glass avoids exposing internal
...
### Glass cannot identify the client
...
to the Glass address with only the intended participants. Contact your service
team promptly if confidential information was sent to an unintended recipient.

### Glass asked for missing information
