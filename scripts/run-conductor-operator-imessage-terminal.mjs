...
for (const relativePath of [
  ".context/operator-imessage-worker.env",
  ".convex/local/default/config.json",
  "imessage-worker/dist/index.js",
]) {
  if (!existsSync(path.join(repoRoot, relativePath))) {
    throw new Error(
      `${relativePath} is missing. Run npm run conductor:setup before starting the operator Spectrum terminal.`,
    );
  }
}

const { operatorImessage } = conductorPorts();
const { site } = await waitForLocalConvex();
const workerEnv = Object.fromEntries(
  parseEnvFile(path.join(repoRoot, ".context", "operator-imessage-worker.env")),
);
const terminalEnvironment = {
  ...
  PORT: String(operatorImessage),
  WORKER_HTTP_PORT: String(operatorImessage),
  NO_COLOR: "1",
};
