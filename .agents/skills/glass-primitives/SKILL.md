...
- Policies and broker follow-ups: `convex/policies.ts`, `convex/lib/policyVersioning.ts`, `convex/lib/policyLookup.ts`, and `convex/lib/policyTypes.ts`; policy-update requests use the generic broker-email path instead of policy-change case helpers.
- Certificates: `convex/certificates.ts`, `convex/lib/workflows/certificateRequest.ts`, `convex/lib/certificateRequestGate.ts`, `convex/lib/certificateHolderPopulation.ts`, `convex/lib/certificateIdentity.ts`, `convex/lib/certificateDescription.ts`, and `convex/lib/coiGenerator.ts`.
